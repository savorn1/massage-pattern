...
export class WorkplacesController {
  ...
  async create(
    ...
    return this.workplacesService.createWorkplace(
      createWorkplaceDto,
      req.user.id,
    );
  }
  ...
  async findAll(
    ...
    return this.workplacesService.getWorkplacesByMember(
      req.user.id,
      skip,
      limit,
    );
  ...
  async getStats(@Request() req: AuthenticatedRequest) {
    const result = await this.workplacesService.getWorkplacesByMember(
      req.user.id,
      0,
      100,
    );
    const workplaceIds: string[] = (
      result.data as { _id: { toString(): string } }[]
  ...
  async findBySlug(
    ...
    const access = await this.workplacesService.hasAccess(
      String(workplace._id),
      req.user.id,
    );
    if (!access)
      throw new ForbiddenException('You are not a member of this workplace');
    return workplace;
  }
  ...
  async update(
    ...
    const allowed = await this.workplacesService.isAdminOrOwner(
      id,
      req.user.id,
    );
    if (!allowed)
      throw new ForbiddenException(
        'Only workspace admins or owners can update',
      );
  ...
  async remove(@Param('id') id: string, @Request() req: AuthenticatedRequest) {
    ...
    if (!allowed)
      throw new ForbiddenException(
        'Only workspace admins or owners can delete',
      );
