  ...
  private subscribeAll() {
    ...
    this.nats.subscribe(TASK_EVENTS.COMMENT_ADDED, async (raw) => {
      try {
        ...
        this.logger.debug(
          `[task.comment.added] activity logged for task ${event.taskId}`,
        );
      } catch (err) {
    ...
    this.nats.subscribe(TASK_EVENTS.CREATED, async (raw) => {
      try {
        const event = JSON.parse(raw) as TaskCreatedEvent;
        await this.activitiesService.logActivity(
          event.taskId,
          event.userId,
        ...
        this.logger.debug(
          `[task.created] activity logged for task ${event.taskId}`,
        );
      } catch (err) {
    ...
    this.nats.subscribe(TASK_EVENTS.UPDATED, async (raw) => {
      try {
        ...
        this.logger.debug(
          `[task.updated] activities logged for task ${event.taskId}`,
        );
      } catch (err) {
    ...
    this.nats.subscribe(TASK_EVENTS.FILE_ATTACHED, async (raw) => {
      try {
        ...
        this.logger.debug(
          `[task.file.attached] activity logged for task ${event.taskId}`,
        );
      } catch (err) {
